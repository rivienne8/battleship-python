...
def get_move(board, player_name, user_input):
    ...
    while row < len(board) or col == 100:
        if count > 0:
...
def get_placing_move(board,player_name,ship_length=0,direction="",color =""):
    row, col = 100, 100
    while row == 100 or col == 100:
        ...
        if  direction == "vertical":  # trzeba jakoś podzielić get_move albo wydzielić osobną dla placement_place
            try:
...
def main_menu():
    # global variables
    setup_dict_5 = {"ships_number" : 3, "ships_sizes": [3,3,4], "board_sizes" : 5}
    setup_dict_6 = {"ships_number" : 3, "ships_sizes": [3,4,5], "board_sizes" : 6}
